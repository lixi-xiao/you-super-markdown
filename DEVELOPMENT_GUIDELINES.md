...
**所有更新包只存放于 `工作区\update-package`**，禁止在其他目录（如各源码目录内）创建 `update-package` 或类似目录。
...
3. 功能/修复代码就绪后，按 3.3/3.4 制作更新包到 `工作区\update-package`
4. 更新 `工作区\update-package\version.json`
5. 打 git tag 并推送
