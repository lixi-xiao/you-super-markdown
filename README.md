...
不管哪条路，流程都是：**备份 → 停守护 → 应用文件 → 重启守护 → 记日志**，全程自动。更新包在应用前会做**完整性校验**（签名 + 逐文件比对），被篡改过的包会被直接拒绝。升级完成后系统会自动发邮件告诉管理员本次改了什么。

如果升级中途失败，系统会**自动回滚**到更新前的备份，并把"已自动回退"写进更新状态，同时发邮件通知管理员——正常使用基本不用担心更新把站搞挂。
