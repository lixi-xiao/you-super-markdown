...
一个轻量级的 Markdown 在线阅读、写作与评论平台，当前版本 v4.5.0。
...
- **登录安全（v4.5.0 强化）**：登录态与环境指纹绑定（浏览器语言/时区/分辨率/绘制指纹），换浏览器或设备必须重新登录；同账号新登录自动踢掉旧会话；登录态过期后可用长效凭证自动续期（换环境则失效）；敏感操作和评论均做环境校验，异常环境直接被拦。
