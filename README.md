...
一个轻量级的 Markdown 在线阅读、写作与评论平台，当前版本 v4.7.3。
...
- ~~**QQ 音乐播放通道（含 QQ 歌单/QQ Cookie/通道切换滑块）**~~：v4.3.0 引入双平台通道切换；QQ 音乐登录授权（Cookie）约 2 小时即过期需频繁更换，服务器端自动申请 Cookie 违反用户协议不可取，v4.4.2 彻底移除，播放器仅保留网易云通道，并保留「默认播放歌曲」配置（网易云歌单内按歌曲名自动定位播放）。
