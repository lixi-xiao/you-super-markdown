...
一个轻量级的 Markdown 在线阅读、写作与评论平台，当前版本 v4.6.0。
...
- **登录安全（v4.5.0/v4.6.0 强化）**：登录态与环境指纹绑定（浏览器语言/时区/分辨率/绘制指纹），换浏览器或设备必须重新登录；同账号新登录自动踢掉旧会话；前台登录态长效 30 天、站长/写作者后台 24 小时过期回退首页重进（按登录时间算）、超管严格 30 分钟（无长效续期，到期必须重新 OTP 登录）；会话 Cookie 已启用 HttpOnly + SameSite=Strict + Secure；敏感操作和评论均做环境校验，异常环境直接被拦。
