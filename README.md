...
一个轻量级的 Markdown 在线阅读、写作与评论平台，当前版本 v3.2.3。
...
- **ZIP 批量上传文章**（写作界面）：支持 `.zip` 解压批量导入 Markdown 文档。已内置多重校验——**防解压炸弹**（文件数 ≤200、解压总大小 ≤50MB、单文件 ≤10MB，超限拒绝导入）、**类型白名单**（仅接受 `.md` / `.txt` / `.markdown`，其他类型一律跳过）、**文件名净化**（仅保留字母/数字/下划线/连字符/中文，防路径穿越）、隐藏文件跳过、CSRF 校验。不会成为攻击入口。
...
**SMTP 没配置也能收到邮件吗？** 收不到。告警/更新通知都走 SMTP，去后台「邮件设置」配置并测试；没配的话失败会记录到服务器日志，不阻断系统运行。
