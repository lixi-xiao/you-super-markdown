...
一个轻量级的 Markdown 在线阅读、写作与评论平台，当前版本 v3.3.7。
...
- **CLI 快捷修改 SMTP 授权码（v3.3.7）**：`sudo ym-admin set-smtp-pass`（或 `--pass=授权码`）一条命令完成——同时更新 root 密钥文件与 php-fpm 环境变量并 reload，改完自动发测试邮件验证，无需再手动改两处；写审计日志且不落盘授权码明文。
