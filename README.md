...
一个轻量级的 Markdown 在线阅读、写作与评论平台，当前版本 v4.9.1。
...
- **阅读**：Markdown 文章在线渲染，代码高亮、侧边目录、夜间模式都有，手机上看也不难受；深色模式还能跟随系统自动切换。正文排版能力包括：**代码块语法高亮（常见语言，带复制按钮）**、**mermaid 图表（流程图/时序图/状态图等，文本即图）**、**LaTeX 数学公式（$ 行内 / $$ 块级，兼容 \(...\)/\[...\] 原生写法，v4.9.0 起；含公式的文章按需加载引擎，无公式文章零额外开销）**、图片灯箱、表格与长代码横向滚动、阅读进度与章节目录锚点。
- **写作**：后台直接写 Markdown，能插图、能插视频、能分分类、打标签；写完可以直接发布，也可以先存草稿、或定个时间自动发布。
- **搜索与归档**：站内全文搜索（标题、标签、摘要、正文都能搜）；按年月归档浏览，点标签还能聚合看同类文章。
- **订阅**：内置 RSS 订阅，读者可以用任何阅读器追更。
- **评论**：注册了才能发言（或开放访客评论），站长可以随时开关评论；新评论还能邮件订阅通知，不落一条互动。
...
- **登录安全（v4.5.0/v4.6.0/v4.7.x 强化）**：登录态与环境指纹绑定（浏览器语言/时区/分辨率/绘制指纹），换浏览器或设备必须重新登录；同账号新登录自动踢掉旧会话；前台登录态长效 30 天、站长/写作者后台 24 小时过期回退首页重进（按登录时间算）、超管严格 30 分钟（无长效续期，到期必须重新 OTP 登录）；会话 Cookie 已启用 HttpOnly + SameSite=Strict + Secure；敏感操作和评论均做环境校验，异常环境直接被拦；**v4.7.0 起管理角色（超管/站长/写作者）陌生设备登录增加邮箱验证码二次确认**，验证通过后设备记为已信任（超管后台可管理）；**联动威胁评分**把 11 类安全事件统一计分，同一 IP/浏览器指纹累计达阈值自动联动封锁（50→15分钟 / 100→24小时 / 200→永久）；首页「忘记密码」支持找回（管理角色仅常用设备可自助，陌生设备联系超管）；触发联动封锁与密码重置均自动邮件告警（统一 HTML 模板）。
- **本地背景音乐（v4.5.0）**：站长/超管后台上传一首歌（mp3/wav/flac 等 ≤100MB，服务器自动转码压缩为 96kbps mp3）作为站点背景音；前台访客只能在播放器弹窗内开/关，不能选曲；首次播放后缓存在浏览器本地，之后几乎不再消耗服务器流量。
...
## 已放弃功能（曾尝试实现、确认无法稳定实现，已移除；不提供对应更新包/下载）

以下功能曾尝试实现，但经实测确认无法稳定实现或效果不达预期，已从当前版本移除。历史描述保留在项目更新日志中（以删除线标记），此处一并列出以免误导：

- ~~**卡片玻璃效果（毛玻璃/液态玻璃切换 + 用户个人开关）**~~：v4.1.16 引入；液态玻璃显示无法稳定实现（手机端背景放大、边缘折射效果不达标），v4.2.0 整体移除，卡片恢复简洁半透明风格。
- ~~**卡片封面 API 配置栏**~~：v4.1.13 引入；v4.2.0 移除，封面固定走服务端封面池。
- ~~**v4.2.0 卡片封面直连外部图片源**~~：外部图片源一次只能取一张图，多卡并行直连触发限流（429）致部分卡片无封面、手机加载慢；v4.2.1 回退服务端封面池（960px 缩略图）。
- ~~**人机滑块验证**~~：v2.9.0 引入；v2.11.0 彻底移除，注册仅保留邮箱验证码验证。
- ~~**QQ 音乐播放通道（含 QQ 歌单/QQ Cookie/通道切换滑块）**~~：v4.3.0 引入双平台通道切换；QQ 音乐登录授权（Cookie）约 2 小时即过期需频繁更换，服务器端自动申请 Cookie 违反用户协议不可取，v4.4.2 移除 Cookie 通道；**v4.7.4 以官方榜单 + 第三方播放地址解析接口重新支持 QQ 音乐（免 Cookie，128k/320k），同时新增酷狗音乐**。
- ~~**「隐藏管理员」称谓**~~：v2.6.5 引入；v2.6.6 恢复为「高级管理员」（凭据仍不展示）。
...
- **公告站顶玻璃拟态（v3.3.16）**：首页公告头改为半透明 + 高斯模糊玻璃效果（桌面/手机端），不再突兀；公告卡片类型徽章移到标题旁边（同一行）。
- **密码眼睛线稿图标（v3.3.16）**：登录/注册密码显示切换按钮由 emoji 👁 换成线稿 SVG 两态图标（睁眼/划线），并补齐了切换逻辑（原按钮点击无反应）。
- **阅读页大图自动转缩略图（v3.3.17）**：文章阅读页里超过 3MB 的站内图片自动改写成缩略图（宽 1600px、产物 ≤2MB）——富媒体文章（航拍截图等）点进不再拖慢加载，原图保留但页面不再请求；gif 动图跳过。
- **手机端评论区头像修复（v3.3.17）**：评论区头像改为固定尺寸（不依赖父容器高度传递）——修复部分手机浏览器上头像变形/显示成方形的问题。
...
本软件按"现状"提供，作者不对其适用性、可靠性或完整性作出任何明示或暗示的保证。使用者在使用本软件及其相关部署过程中产生的任何直接、间接、偶然或后果性损失（包括但不限于数据丢失、服务中断、经济损失等），作者均不承担任何责任。

本软件为自托管轻量解决方案，请在使用前自行做好数据备份与安全加固，并遵守所在地区法律法规。源码采用 MIT 许可，使用者可自由使用、修改与分发，但需自行承担相应风险与责任。
