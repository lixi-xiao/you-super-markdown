(function() {
    ...
    // v4.1.18-fix：液态玻璃「边缘折射变形」Canvas 真扭曲增强
    // 后台选液态玻璃 + 有背景 + 用户未在个人开关关闭 → 自动启用（默认开，无需独立开关）；
    // 关闭液态玻璃 / 毛玻璃模式 / 无背景 / 浏览器不支持 Canvas → 自动停用，纯 CSS 玻璃兜底。
    // 渲染范围：全部玻璃家族；边缘位移最大 7px（中等），中心趋零；降采样 + 防抖保证性能。
    (function initGlassRefraction() {
        var body = document.body;
        var canvas = null, ctx = null, bgImg = null, started = false;
        var MAX_W = 1600;            // 全屏画布宽度上限（性能保护）
        var EDGE_AMP = 7;            // 边缘最大位移 px
        var BLOCK = 10;              // 位移块大小
        var GLASS_SEL = ['.doc-card', '.markdown-body', '.comment-section', '.cmt-input-box', '.prev-next-nav', '.share-card', '.article-info-card', '.cmt-capsule-btn', '.cmt-user-inner'].join(',');
        ...
        function active() {
            return body.classList.contains('glass-liquid')
                && body.classList.contains('bg-active')
                && bgImg && bgImg.complete && bgImg.naturalWidth > 0;
        }
        function bgUrl() {
            var u = body.getAttribute('data-bg-image') || '';
            if (!u) return '';
            return (u.indexOf('/') === 0 || /^https?:/i.test(u)) ? u : '/' + u;
        }
        ...
        function distortRect(x, y, w, h, kx, ky) {
            ...
            var sx = Math.max(0, Math.round(x * kx));
            var sy = Math.max(0, Math.round(y * ky));
            var imgData;
            try { imgData = ctx.getImageData(sx, sy, cw, ch); } catch (e) { return; }
            var off = document.createElement('canvas');
            off.width = cw; off.height = ch;
            off.getContext('2d').putImageData(imgData, 0, 0);
            ctx.save();
            ctx.beginPath(); ctx.rect(sx, sy, cw, ch); ctx.clip();
            for (var by = 0; by < ch; by += BLOCK) {
                for (var bx = 0; bx < cw; bx += BLOCK) {
                    ...
                    var dy = Math.round(Math.cos(ph * 1.3) * amp);
                    ctx.drawImage(off, bx, by, BLOCK, BLOCK, bx + dx, by + dy, BLOCK, BLOCK);
                }
            }
            ctx.restore();
        ...
        function render() {
            renderTimer = null;
            if (!active()) { if (canvas) canvas.style.display = 'none'; return; }
            var W = body.clientWidth, H = body.clientHeight;
            if (!W || !H) return;
            var cw = Math.min(W, MAX_W), ch = Math.round(cw * H / W);
            ...
            for (var i = 0; i < els.length; i++) {
                ...
                if (r.bottom < 0 || r.top > H || r.right < 0 || r.left > W) continue; // 视口外跳过
                if (r.width < 8 || r.height < 8) continue;
                distortRect(r.left, r.top, r.width, r.height, kx, ky);
            }
        }
        function schedule() {
            if (!active()) {
                body.classList.remove('glass-refract');
                if (canvas) canvas.style.display = 'none';
                return;
            }
            body.classList.add('glass-refract');
            if (!canvas || renderTimer) return;
            renderTimer = setTimeout(function() { requestAnimationFrame(render); }, 120);
        }
        function start() {
            if (started) return;
            started = true;
            ...
            // 用户下拉菜单切换液态玻璃开关时同步（userDropdownGlass 点击由 applyGlass 处理，此处兜底调度）
            document.addEventListener('click', function(e) {
                var n = e.target;
                while (n && n !== document) { if (n.id === 'userDropdownGlass') { schedule(); return; } n = n.parentNode; }
            });
        }
